...
/**
 * script for bulk user multi cohort remove
 *
 * @package    core
 * @subpackage user
...
require_once('user_bulk_cohortremove_form.php');
...
$mform = new user_bulk_cohortremove_form(null, $cohorts);
...
if (empty($users) or $mform->is_cancelled()) {
    ...
    foreach ($users as $user) {
        if ($DB->record_exists('cohort_members', array('cohortid' => $data->cohort, 'userid' => $user->id))) {
            cohort_remove_member($data->cohort, $user->id);
        }
    }
    redirect($return);
}
...
foreach ($columns as $column) {
    ...
    $table->head[] = '<a href="user_bulk_cohortremove.php?sort='.$column.'&amp;dir='.$columndir.'">'.$strtitle.'</a>'.$columnicon;
    $table->align[] = 'left';
}
...
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('bulkremove', 'core_cohort'));
